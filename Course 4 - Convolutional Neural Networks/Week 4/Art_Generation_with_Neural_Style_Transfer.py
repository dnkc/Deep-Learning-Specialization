...
def compute_layer_style_cost(a_S, a_G):
    ...
    #     J_content = (1/(4*n_H*n_W*n_C))*tf.reduce_sum(tf.square(tf.subtract(a_C, a_G)))
    J_style_layer = 1/(4*np.square(n_C)*np.square((n_H*n_W)))*tf.reduce_sum(tf.reduce_sum(tf.square(tf.subtract(GS,GG))))
    #J_style_layer = tf.cast(J_style_layer, tf.float32)
    print(J_style_layer)
    ### END CODE HERE
...
def train_step(generated_image):
    with tf.GradientTape() as tape:
        ...
        #(1 line)
        # compute_style_cost(style_image_output, generated_image_output, STYLE_LAYERS=STYLE_LAYERS):
